...
contract ChainLinkPriceFeed is Ownable{
    ...
    function addToken(string calldata tokenName , address primaryFeedAddress ,address secondaryFeedAddress , int256 intialPrice , uint8  decimals) external onlyOwner{
    ...
    function getPrice(string calldata tokenName ) external returns(int256 )  {
        ...
        if(roundID != 0 && price >=0 && updatedAt <= block.timestamp && (block.timestamp - updatedAt) <TIMEOUT && _absoluteValue(price - feed.lastGoodPrice) < MAX_ALLOWED_DEVIATION ){
