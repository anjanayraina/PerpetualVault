...
contract PerpetualVaultHarness is PerpetualVault{
    ...
    function _calculatePositionOpeningFeeInternal(uint positionSize)  public view returns(uint256){
    ...
    function _absouluteValueInternal(int value) public  pure returns(uint256){
        return super._absoluteValue(value);
    }

    function _canChangeSizeInternal(bytes32 positionID, uint256 sizeChange, bool isIncerement) public view returns (bool) {
        return super._canChangeSize(positionID , sizeChange , isIncerement);
    }

    function _canChangeCollateralInternal(bytes32 positionID, uint256 sizeChange, bool isIncement) public view returns(bool){
        return super._canChangeCollateral(positionID , sizeChange , isIncement);
    }

    function _getPositionHashInternal(address owner , uint256 collateralInUSD , uint256 sizeInUSD , bool isLong) public pure returns(bytes32){
